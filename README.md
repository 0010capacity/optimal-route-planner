...
**완전탐색 (≤6개 경유지)**
- 모든 가능한 순열 조합 계산으로 최적해 보장
- 6! = 720가지 조합까지 실시간 처리 가능

**2-opt 최적화 (7-12개 경유지)**
- 거리 행렬 구축 후 지역 최적화 수행
- Nearest Neighbor 초기해 + 2-opt 개선
- 평균 10-20회 반복으로 수렴

**휴리스틱 최적화 (>12개 경유지)**
- 거리 기반 샘플링으로 핵심 지점 추출
- 클러스터링과 2-opt 조합으로 대용량 처리

### 성능 지표
- **처리 시간**: 6개 경유지 기준 평균 2-3초
- **API 효율성**: 초당 평균 5-10회 호출
- **메모리 사용량**: 10MB 이하 유지
- **정확도**: 소규모(≤6개)에서 100% 최적해, 중대규모에서 95%+ 근사해
